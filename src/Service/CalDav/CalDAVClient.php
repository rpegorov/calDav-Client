...
use Services\CalDav\Utils\XMLDocument;
use Services\CalDav\Utils\XMLElement;
...
protected $parser;
  ...
  function DoOptionsRequest( $url = null ) {
    ...
    /**
     * Send an XML request to the server (MKCALENDAR)
     *
     * @param string $name The calendar name
     * @param string $displayName The calendar display name
     * @return array An array of the allowed methods
     */
  function CreateCalendar(string $name, string $displayName){
      $xml_text = <<<EOXML
        ...
        EOXML;
      return $this->DoXMLRequest('MKCALENDAR', $xml_text, $this->first_url_part."/calendars/".$this->user."/".$name."/");

  }
    /**
     * Send an XML request to the server (REPORT)
     * @param string $token The calendar sync token
     * @return array An array of the allowed methods
     */
  function SyncCalendar(string $token){
      $xml_text = <<<EOXML
        ...
        EOXML;
      $this->DoXMLRequest('REPORT', $xml_text, $this->calendar_url);
      return $this->httpResponseBody;
  ...
  function FindCalendars( $recursed=false ) {
      ...
      return $this->DoPROPFINDRequest( $this->first_url_part.$this->calendar_home_set[0], $properties, 1);
  }
  ...
  function GetCalendarDetailsByURL($url) {
      ...
      return $this->DoPROPFINDRequest($url, $properties, 0);
